...
const UNIT_LABELS: Record<UnitKey, string> = {
    "m/s²": "Meter per second squared (m/s²)",
    "cm/s²": "Centimeter per second squared (cm/s²)",
    "mm/s²": "Millimeter per second squared (mm/s²)",
    "km/s²": "Kilometer per second squared (km/s²)",
    "ft/s²": "Foot per second squared (ft/s²)",
    "in/s²": "Inch per second squared (in/s²)",
    "ft/min²": "Foot per minute squared (ft/min²)",
    "in/min²": "Inch per minute squared (in/min²)",
    "km/h/s": "Kilometer per hour per second (km/h/s)",
    "mi/h/s": "Mile per hour per second (mi/h/s)",
    "g": "Standard gravity (g)",
    "Gal": "Galileo (Gal)",
    "mGal": "Milligalileo (mGal)",
};

function AccelerationConverter(): JSX.Element {
    const [value, setValue] = useState<string>("1");
    const [fromUnit, setFromUnit] = useState<UnitKey>("m/s²");
    const [toUnit, setToUnit] = useState<UnitKey>("ft/s²");
    ...
    const sampleSteps = useMemo(() => {
        ...
        return [
            `${raw} ${UNIT_LABELS[fromUnit]} × ${UNIT_FACTORS[fromUnit].toLocaleString()}  = ${pa.toLocaleString()} m/s²`,
            `${pa.toLocaleString()} m/s² ÷ ${UNIT_FACTORS[toUnit].toLocaleString()}  = ${toVal.toLocaleString(undefined, { maximumFractionDigits: precision })} ${toUnit}`,
        ];
        ...
        <div className="bg-gradient-to-r from-slate-50/80 via-blue-50/60 to-teal-50/80 dark:from-gray-900/80 dark:via-gray-800/60 dark:to-gray-900/80 transform-all duration-300 transition:ease-in-out">
            ...
            <div className="ml-10  mt-3">
                <p className="text-xs dark:text-white ">
                    ...
                    &gt;
                    <Link to='/All-calculators/Unit-Conversions/Acceleration-Converter'>
                        <span className='hover:underline'>  Acceleration Converter </span>
                    </Link>
                </p>
            </div>
            <div className="min-h-screen flex items-center justify-center p-4 ">
                <div className="max-w-6xl w-full bg-lime-200/20 dark:bg-lime-800/20 rounded-2xl shadow-lg overflow-hidden border border-lime-300 dark:border-lime-700 transition-all duration-300 ease-in-out">
                    <div className="p-4 md:p-6 grid gap-4 md:grid-cols-3">
                        <div className="md:col-span-2">
                            <div className="flex items-center justify-between mb-4">
                                <h1 className="text-xl md:text-2xl font-semibold text-slate-800 dark:text-slate-100">Acceleration Converter</h1>
                                ...
                                <div className="flex items-end gap-3">
                                    ...
                                    <div className="w-28">
                                        <button
                                            ...
                                            className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 border border-lime-300 dark:border-lime-600 text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                                        >
                                            Reset
                                        </button>
                                    </div>
                        ...
                        <aside className="md:col-span-1 border-l dark:border-slate-700 pl-4">
                            <div className="sticky top-6">
                                ...
                                <div className="p-3 rounded-lg bg-slate-50 dark:bg-gray-700 border dark:border-slate-700 dark:text-gray-200 mb-4 text-sm">
                                    {sampleSteps.map((s, i) => (
                                ...
                                <div className="p-3 rounded-lg bg-slate-50 dark:bg-gray-700 border dark:border-slate-700 text-xs dark:text-gray-200">
