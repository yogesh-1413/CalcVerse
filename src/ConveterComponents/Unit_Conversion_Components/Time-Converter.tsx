...
const UNIT_LABELS: Record<UnitKey, string> = {
    ...
    "day": "Day (day)",
    "week": "Week (week)",
    "month (30d)": "Month (30 days)",
    "year (365d)": "Year (365 days)",
 ...
 function TimeConverter(): JSX.Element {
  ...
  const [fromUnit, setFromUnit] = useState<UnitKey>("day");
  const [toUnit, setToUnit] = useState<UnitKey>("min");
  ...
  const sampleSteps = useMemo(() => {
    ...
    return [
      `${raw} ${UNIT_LABELS[fromUnit]} × ${UNIT_FACTORS[fromUnit].toLocaleString()}  = ${pa.toLocaleString()} day`,
`${pa.toLocaleString()} day  ÷ ${UNIT_FACTORS[toUnit].toLocaleString()}= ${toVal.toLocaleString(undefined, { maximumFractionDigits: precision })} ${toUnit}`,
    ];
  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center p-4 ">
      <div className="max-w-6xl w-full bg-emerald-200/20 dark:bg-emerald-800/20 rounded-2xl shadow-lg overflow-hidden border border-emerald-300 dark:border-emerald-700 transition-all duration-300 ease-in-out">
        <div className="p-4 md:p-6 grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-xl md:text-2xl font-semibold text-slate-800 dark:text-slate-100">Time Converter</h1>
              ...
              <div className="flex items-end gap-3">
                ...
                <div className="w-28">
                  <button
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 border border-emerald-300 dark:border-emerald-600 text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                  >
                    Reset
                  </button>
                </div>
