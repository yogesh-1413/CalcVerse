...
const UNIT_FACTORS: Record<UnitKey, number> = {
  ...
  "mpg_US": 235.214583,             
  "mpg_UK": 282.480936,
  "galUS_per_100mi": 6.092029742109697,
  "galUK_per_100mi": 7.31622266496,
};

const UNIT_LABELS: Record<UnitKey, string> = {
    "L_per_100km": "Liters per 100 kilometers (L/100km)",  
    "Kmpl": "Kilometers per liter (km/L)",
    "mpg_US": "Miles per gallon (US) (mpg)",
    "mpg_UK": "Miles per gallon (UK) (mpg)",
    "galUS_per_100mi": "Gallons (US) per 100 miles (gal/100mi)",
    "galUK_per_100mi": "Gallons (UK) per 100 miles (gal/100mi)",

};

 function FuelConsumption(): JSX.Element {
  const [value, setValue] = useState<string>("1");
  const [fromUnit, setFromUnit] = useState<UnitKey>("Kmpl");
  const [toUnit, setToUnit] = useState<UnitKey>("mpg_US");
  ...
  const sampleSteps = useMemo(() => {
    ...
    return [
      `${raw} ${UNIT_LABELS[fromUnit]} × ${UNIT_FACTORS[fromUnit].toLocaleString()}  = ${pa.toLocaleString()} Kmpl`,
      `${pa.toLocaleString()} Kmpl ÷ ${UNIT_FACTORS[toUnit].toLocaleString()}= ${toVal.toLocaleString(undefined, { maximumFractionDigits: precision })} ${toUnit}`,
    ];
  ...
  return (
    <div className="bg-gradient-to-r from-slate-50/80 via-blue-50/60 to-teal-50/80 dark:from-gray-900/80 dark:via-gray-800/60 dark:to-gray-900/80 transform-all duration-300 transition:ease-in-out">
        ...
        <p className="text-xs dark:text-white ">
          ...
          &gt;
          <Link to='/All-calculators/Unit-Conversions/Fuel-Consumption-Converter'>
            <span className='hover:underline'>  Fuel Consumption Converter </span>
          </Link>
        </p>
      </div>
    <div className="min-h-screen flex items-center justify-center p-4 ">
      <div className="max-w-6xl w-full bg-amber-200/20 dark:bg-amber-800/20 rounded-2xl shadow-lg overflow-hidden border border-amber-300 dark:border-amber-700 transition-all duration-300 ease-in-out">
        <div className="p-4 md:p-6 grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-xl md:text-2xl font-semibold text-slate-800 dark:text-slate-100">Fuel Consumption Converter</h1>
              ...
              <div className="flex items-end gap-3">
                ...
                <div className="w-28">
                  <button
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 border border-amber-300 dark:border-amber-600 text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                  >
                    Reset
                  </button>
                </div>
          ...
          <aside className="md:col-span-1 border-l dark:border-slate-700 pl-4">
            <div className="sticky top-6">
              ...
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-gray-700 border dark:border-slate-700 mb-4 text-sm dark:text-gray-200">
                {sampleSteps.map((s, i) => (
