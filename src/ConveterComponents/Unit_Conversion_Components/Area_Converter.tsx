...
const UNIT_LABELS: Record<UnitKey, string> = {
 ...
 function AreaConverter(): JSX.Element {
  ...
  const sampleSteps = useMemo(() => {
    ...
    return [
      `${raw} ${UNIT_LABELS[fromUnit]} × ${UNIT_FACTORS[fromUnit].toLocaleString()}  = ${pa.toLocaleString()} m²`,
      `${pa.toLocaleString()} m² ÷ ${UNIT_FACTORS[toUnit].toLocaleString()}  = ${toVal.toLocaleString(undefined, { maximumFractionDigits: precision })} ${toUnit}`,
    ];
  ...
  return (
    <div className="bg-gradient-to-r from-slate-50/80 via-blue-50/60 to-teal-50/80 dark:from-gray-900/80 dark:via-gray-800/60 dark:to-gray-900/80 transform-all duration-300 transition:ease-in-out">
        ...
        <p className="text-xs dark:text-white ">
          ...
          &gt;
          <Link to='/All-calculators/Unit-Conversions/Area-Converter'>
            <span className='hover:underline'>  Area Converter </span>
          </Link>
        </p>
      </div>
    <div className="min-h-screen flex items-center justify-center p-4 ">
      <div className="max-w-6xl w-full bg-blue-200/20 dark:bg-blue-800/20 rounded-2xl shadow-lg overflow-hidden border border-blue-300 dark:border-blue-700 transition-all duration-300 ease-in-out">
        <div className="p-4 md:p-6 grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-xl md:text-2xl font-semibold text-slate-800 dark:text-slate-100">Area Converter</h1>
              ...
              <div className="flex items-end gap-3">
                ...
                <div className="w-28">
                  <button
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 border border-blue-300 dark:border-blue-600 text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                  >
                    Reset
                  </button>
                </div>
